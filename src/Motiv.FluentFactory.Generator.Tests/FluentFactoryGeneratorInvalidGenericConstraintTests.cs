...
public class FluentFactoryGeneratorInvalidGenericConstraintTests
{
    ...
    /// <see cref="ExpectedDiagnostics"/> and asserts no MFFG diagnostic is emitted and no
    ...
    internal async Task Should_not_crash_or_emit_mffg_diagnostics_when_user_source_has_invalid_constraint_combination()
    {
        ...
        // The `struct` keyword appears at the end of `where T : class, struct` on line 8.
        await new VerifyCS.Test
        {
            TestState =
            {
                ...
                ExpectedDiagnostics =
                {
                    DiagnosticResult.CompilerError("CS0449")
                        .WithSpan("Source.cs", 8, 51, 8, 57),
                }
                // No GeneratedSources listed — generator should produce no output (or its output is
                // not asserted) since the goal is that no MFFG error fires and no exception is thrown.
            }
        }.RunAsync();
    }
}
